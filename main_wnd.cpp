...
void MainWnd::VideoRenderer::OnFrame(
	const webrtc::VideoFrame& video_frame) {
		...
		{
			uint32_t prevTimestamp = video_frame.timestamp();
			rtc::scoped_refptr<webrtc::I420Buffer> vframe;
			webrtc::VideoFrame trans_frame(vframe, webrtc::kVideoRotation_0, prevTimestamp);
			video_frame.video_frame_buffer() = trans_frame.video_frame_buffer();
			//SetSize(video_frame->width(), buffer->height());
			//OnFrame(trans_frame);
			/*AutoLock<VideoRenderer> lock(this);
